...
$("#shredderLG").click(upgradeItem(industrialUpgradeInitial, industrialUpgradeMultiplier, numIndustrials, industrialEffcacy, #shredderLG, #industrialCost));
...
function upgradeItem(initial, multiplier, numItems,  effcacy, itemID, itemCostID){
  if (gameScore >= initial*multiplier){
    ...
    //update economy text
    $('#scoreboard').html('Score: '+gameScore);
    $(itemID).html(numIndustrials);
    $(itemCostID).html(('-')+newCost);
  }
    numItems = numItems++;
});
